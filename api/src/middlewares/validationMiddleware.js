...
async function validationMiddleware(req, res, next) {
  ...
  if(weightMax<weightMin) return res.status(400).json({msg:"The maximum weight cannot be lower than the minimum weight!"})
  if(heightMax<heightMin) return res.status(400).json({msg:"The maximum height cannot be lower than the minimum height!"})
